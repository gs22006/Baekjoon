...
void dfs4(ll here, ll pa)
{
	for(auto &i : vec[here])
	{
		if(i.fi == pa || R[i.fi] || chk[i.fi] != cc)
			continue;
		
		dist[i.fi] = dist[here] + i.se;
		dfs4(i.fi, here);
	}
}
...
void f(ll here, vector<ll> qry)
{
	...
	for(auto &i : vec[rt])
	{
		...
		maxi = MIN;
		dfs2(i.fi, rt);
		LF.push_back({mini, i.fi});
		LF.push_back({maxi, i.fi});
	}
